# File Tree: AI_Engineer-DL

**Root Path:** `d:\AI_Engineer-DL`

```
├── 01_neural_networks_and_dl
│   ├── 01_intro_to_dl
│   │   ├── exercises
│   │   │   ├── 01_what_is_a_neural_network_ex1_relu_from_scratch.py
│   │   │   ├── 01_what_is_a_neural_network_ex2_single_neuron.py
│   │   │   └── 01_what_is_a_neural_network_ex3_two_layer_network.py
│   │   ├── images
│   │   │   ├── 01_relu_vs_sigmoid.png
│   │   │   └── 01_single_neuron_housing.png
│   │   ├── notebooks
│   │   │   └── 01_what_is_a_neural_network.ipynb
│   │   ├── notes
│   │   │   └── 01_what_is_a_neural_network_notes.md
│   │   └── README.md
│   ├── 02_logistic_regression
│   │   ├── exercises
│   │   ├── images
│   │   │   ├── 01_binary_classification_data_layout.png
│   │   │   ├── 02_logistic_regression_sigmoid.png
│   │   │   ├── 03_gradient_descent_cost.png
│   │   │   ├── 04_computation_graph_backprop.png
│   │   │   └── 05_vectorization.png
│   │   ├── notebooks
│   │   │   ├── 01_binary_classification.ipynb
│   │   │   ├── 02_logistic_regression.ipynb
│   │   │   ├── 03_gradient_descent.ipynb
│   │   │   ├── 04_computation_graph_and_backprop.ipynb
│   │   │   └── 05_vectorization.ipynb
│   │   ├── notes
│   │   │   ├── 01_binary_classification_notes.md
│   │   │   ├── 02_logistic_regression_notes.md
│   │   │   ├── 03_gradient_descent_notes.md
│   │   │   ├── 04_computation_graph_and_backprop_notes.md
│   │   │   └── 05_vectorization_notes.md
│   │   └── README.md
│   ├── 03_shallow_neural_network
│   │   ├── exercises
│   │   ├── images
│   │   ├── notebooks
│   │   ├── notes
│   │   └── README.md
│   ├── 04_deep_neural_network
│   │   ├── exercises
│   │   ├── images
│   │   ├── notebooks
│   │   ├── notes
│   │   └── README.md
│   ├── docs
│   │   └── Deep_Learning_Part_1_What_Is_a_Neural_Network.docx
│   └── README.md
├── 02_improving_deep_nn
│   ├── 01_practical_aspects
│   │   ├── exercises
│   │   ├── images
│   │   ├── notebooks
│   │   ├── notes
│   │   └── README.md
│   ├── 02_optimization_algorithms
│   │   ├── exercises
│   │   ├── images
│   │   ├── notebooks
│   │   ├── notes
│   │   └── README.md
│   ├── 03_hyperparameter_tuning
│   │   ├── exercises
│   │   ├── images
│   │   ├── notebooks
│   │   ├── notes
│   │   └── README.md
│   ├── docs
│   └── README.md
├── 03_structuring_ml_projects
│   ├── 01_ml_strategy_1
│   │   ├── images
│   │   ├── notes
│   │   └── README.md
│   ├── 02_ml_strategy_2
│   │   ├── images
│   │   ├── notes
│   │   └── README.md
│   ├── docs
│   └── README.md
├── 04_convolutional_neural_networks
│   ├── 01_foundations
│   │   ├── exercises
│   │   ├── images
│   │   ├── notebooks
│   │   ├── notes
│   │   └── README.md
│   ├── 02_deep_cnn_models
│   │   ├── exercises
│   │   ├── images
│   │   ├── notebooks
│   │   ├── notes
│   │   └── README.md
│   ├── 03_object_detection
│   │   ├── exercises
│   │   │   └── .gitkeep
│   │   ├── images
│   │   │   └── .gitkeep
│   │   ├── notebooks
│   │   │   └── .gitkeep
│   │   ├── notes
│   │   │   └── .gitkeep
│   │   └── README.md
│   ├── 04_special_applications
│   │   ├── exercises
│   │   │   └── .gitkeep
│   │   ├── images
│   │   │   └── .gitkeep
│   │   ├── notebooks
│   │   │   └── .gitkeep
│   │   ├── notes
│   │   │   └── .gitkeep
│   │   └── README.md
│   ├── docs
│   └── README.md
├── 05_sequence_models
│   ├── 01_recurrent_neural_networks
│   │   ├── exercises
│   │   ├── images
│   │   ├── notebooks
│   │   ├── notes
│   │   └── README.md
│   ├── 02_nlp_word_embeddings
│   │   ├── exercises
│   │   ├── images
│   │   ├── notebooks
│   │   ├── notes
│   │   └── README.md
│   ├── 03_sequence_attention
│   │   ├── exercises
│   │   ├── images
│   │   ├── notebooks
│   │   ├── notes
│   │   └── README.md
│   ├── 04_transformers
│   │   ├── exercises
│   │   ├── images
│   │   ├── notebooks
│   │   ├── notes
│   │   └── README.md
│   ├── docs
│   └── README.md
├── docs
├── projects
│   └── README.md
├── .gitignore
├── AI_Engineer_DL_File Tree.md
├── LICENSE
├── PROGRESS.md
├── README.md
├── ROADMAP.md
└── environment.yml
```